...
public class KunderaQueryTest {
	...
	public void setup() {
		...
		EntityManagerFactory factory = Persistence.createEntityManagerFactory("cassandra_pu", propertyMap);
	...
	public ProductOptionValuePrice populateProductOptionValuePrice(ProductOptionValue pOptValue) throws Exception {
	...
	public Product populateProduct(ProductOptionValue pProductOptionValue,Item pItem) throws Exception {
	...
	public void testQueryProduct() {
		...
		try {
			...
			Product prd = em.find(Product.class,"1000");
	...
	public void assertProductOptionValuePrice(final ProductOptionValuePrice pProductOptionValuePrice) {
		
		assertTrue(pProductOptionValuePrice.getIdProductOptionValuePrice().equals("1005"));
		assertTrue(pProductOptionValuePrice.getItemPrice() != null);
		assertTrue(pProductOptionValuePrice.getItemPrice().getContext() != null);
		assertTrue(pProductOptionValuePrice.getItemPrice().getContext().getIdContext() != null);
		assertTrue(pProductOptionValuePrice.getItemPrice().getContext().getIdContext().equals("1006"));
	...
	public List<Object> findAll(final Class<?> clazz) {
		return  em.createQuery("select ".concat(clazz.getSimpleName().substring(0,1)).concat(" from ")
				.concat(clazz.getSimpleName().concat(" ")
	...
	public void printAll(final Class<?> clazz){
		...
		for (Object p1 : findAll(clazz)) {
			System.out.println("Json: ".concat(String.valueOf(
					m.on(p1).invoke().method("toString").withoutArgs())));
		}
	}
	/**
	 * Multi update method
	 * @param args - Entity classes
	 */
 ...
 * @param args
 */
public void deleteGroup(final Object...args) { 
